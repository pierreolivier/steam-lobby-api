...
exports.login = function(username, encryptedPassword, emailauth, captchaGid, captchaText, timestamp, output) {
    var jar = request.jar();
    var data = {
        username: username,
    ...
    request.post({
        ...
        output(JSON.parse(body), jar.getCookieString('https://steamcommunity.com/'));
    });
};
...
exports.loginErrorMessage = function (captchaNeeded, captchaGid, emailauthNeeded, emailDomain) {
    ...
    return '{"success": false, "message": "user_action", "captcha_needed": ' + captchaNeeded + ', "captcha_gid": "' + captchaGid + '", "emailauth_needed": ' + emailauthNeeded + ', "emaildomain": "' + emailDomain + '"}';
};
...
exports.lobby = function(steamid, cookies, cb) {
    request.get({
        url: 'https://steamcommunity.com/profiles/' + steamid + '/friends/players',
        headers: {
            'Cookie': cookies
        }
    }, function(error, response, body) {
